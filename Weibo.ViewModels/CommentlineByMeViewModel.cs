...
{
    public class CommentlineByMeViewModel : CommentlineViewModel
    {
        #region Overrides of CommentlineViewModel

        public override async void Reload(string token)
        {
            var ses = await WeiboClient.comments_by_me_refresh_async(token, 0, 50, 1, WeiboClient.comments_filter_by_source.all);
            if (ses.Failed())
            {
        ...
        {
            var ses = await WeiboClient.comments_by_me_next_page_async(token,MinId - 1, 50, 1);
