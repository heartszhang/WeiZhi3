...
namespace NReadability
{
  ...
  public class NReadabilityTranscoder
  {
    ...
    private const int _MinParagraphLength = 10;//25;按照英文平均单词长度2.5的规则计算
    private const int _MinInnerTextLength = 25;
    private const int _ParagraphSegmentLength = 17;//中文，平均每句话17个字
    private const int _MaxPointsForSegmentsCount = 3;
      private const int _MinSiblingParagraphLength = 26;//原文80;按照中文计算，3个字母计算一个中文字
    ...
    private const int _MinInnerTextLengthInElementsWithEmbed = 25;//75;for chinese ,we use 25
    private const int _ClassWeightTreshold = 25;
    private const int _MaxEmbedsCount = 1;
      private const int _MaxArticleTitleLength = 35;//150;,中文标题
    private const int _MinArticleTitleLength = 5;//15;//标题中含有15个汉字，很多,但是不知道具体有多少文章的标题会小于10个汉字,暂时按五言绝句
    ...
    private static readonly Regex _VideoRegex = new Regex("http:\\/\\/(www\\.)?(youtube|vimeo|youku|tudou|iqiyi|video\\.sina|tv\\.sohu)\\.com", RegexOptions.Compiled | RegexOptions.IgnoreCase);
    ...
    private static readonly Regex _ArticleTitleDashRegex1 = new Regex(" [\\|\\-_] ", RegexOptions.Compiled);
    private static readonly Regex _ArticleTitleDashRegex2 = new Regex("(.*?)[\\|\\-_].*", RegexOptions.Compiled);
    private static readonly Regex _ArticleTitleDashRegex3 = new Regex("[^\\|\\-_]*[\\|\\-_](.*)", RegexOptions.Compiled);
    ...
    private static readonly Regex _TitleWhitespacesCleanUpRegex = new Regex("\\s+", RegexOptions.Compiled);
      ...
      //对html5网站的支持,不知道theverge.com是什么网站
    private static readonly Dictionary<Regex, string> _articleContentElementHints =
      ...
      private Dictionary<Regex, string> _articleContentDivIdHints2;
      private static readonly Dictionary<Regex, string> _articleContentDivIdHints =
          new Dictionary<Regex, string>
              {
                  {
                      new Regex("^https?://(www|industry)\\.caijing.com.cn",
                                RegexOptions.Compiled | RegexOptions.IgnoreCase), "the_content"
                  },
                  {
                      new Regex("^https?://(www|finance)\\.sina.com.cn", RegexOptions.Compiled | RegexOptions.IgnoreCase),
                      "artibody"
                  },
                  {
                      new Regex("^https?://(www|mobile)\\.xici.net", RegexOptions.Compiled | RegexOptions.IgnoreCase),
                      "text_1_0"
                  },
                  {
                      new Regex("^https?://(www|news)\\.ifeng.com/mainland",
                                RegexOptions.Compiled | RegexOptions.IgnoreCase), "artical_real"
                  },
                  {
                      new Regex("^https?://(www|mobile)\\.tianya.cn", RegexOptions.Compiled | RegexOptions.IgnoreCase),
                      "mark_l"
                  },
                  {
                      new Regex("^https?://(www|mobile)\\.apple.com/cn", RegexOptions.Compiled | RegexOptions.IgnoreCase),
                      "product-review"
                  },
              };
    #endregion
        ...
        Dictionary<Regex, string> divIdHints = null)
    {
      ...
      public NReadabilityTranscoder(ReadingStyle readingStyle, ReadingMargin readingMargin, ReadingSize readingSize,
          Dictionary<Regex, string> divIdHints = null)
      : this(false, false, false, readingStyle, readingMargin, readingSize, divIdHints)
    {
    }
    ...
    /// </summary>
    public NReadabilityTranscoder(Dictionary<Regex, string> divIdHints = null)
      : this(DefaultReadingStyle, DefaultReadingMargin, DefaultReadingSize,divIdHints)
    {
    }
    ...
    internal XDocument TranscodeToXml(string htmlContent, string url, out bool mainContentExtracted, out string extractedTitle, out string nextPageUrl)
    {
      ...
      if (!_dontStripUnlikelys && GetInnerText(articleContentElement).Length < 50)
      {
        try
        {
      ...
      mainContentExtracted = !articleContentElement.IsEmpty;
        extractedTitle =GetInnerText(articleTitleElement,true);//ExtractTitle(document);
    ...
    internal XElement ExtractArticleTitle(XDocument document)
    {
      ...
      if (_ArticleTitleDashRegex1.IsMatch(currentTitle))
      {
        ...
        //中文不能用空格计算字数
        if (NotAsciiCharNorBlanksCount(currentTitle) < _MinArticleTitleWordsCount1)
        {
          currentTitle = _ArticleTitleDashRegex3.Replace(documentTitle, "$1");
        }
      }
      else if ((currentTitle.IndexOf(": ") != -1) || (currentTitle.IndexOf("：") != -1))
      {
    ...
    internal XElement ExtractArticleContent(XDocument document, string url = null)
    {
      ...
      CollapseRedundantParagraphDivs(document);
        ...
        //对html5站点有效，改为针对divid过滤
      string articleContentElementHint =
        !string.IsNullOrEmpty(url)
          ? GetArticleContentElementHintByDivId(url)
          : null;
    ...
    internal XDocument GlueDocument(XDocument document, XElement articleTitleElement, XElement articleContentElement)
    {
        return WriteDocument(articleContentElement);
      //XElement documentBody = GetOrCreateBody(document);
    ...
    XDocument WriteDocument(XElement content)
    {
        ...
        var line = CreateElement("Run");

        for (var node = content.FirstNode; node != null; node = node.NextNode)
        {
            WriteElement(node, ref para, ref line, root, ref linelength, ref breakpending, ref parano);
        }
    ...
    static bool IsBlockLevel(XElement node)
    {        
    ...
    static bool IsLineLevelElement(XElement x)
    {
        return x.Name.LocalName == "li" || x.Name.LocalName == "dt" || x.Name.LocalName == "br" || x.Name.LocalName == "p" || x.Name.LocalName == "pre";
    }
     static   XNamespace _ns="http://schemas.microsoft.com/winfx/2006/xaml/presentation";
    XElement CreateElement(string name)
    {
        return new XElement(_ns + name);
    }
    void WriteElement(XNode e, ref XElement paragraph, ref XElement line, XElement root
        , ref int linelength,ref bool breakpending, ref int parano)
    {
        var content = GetInnerText(e, false);//NormalizeString(x.InnerText);
        if (e.NodeType != XmlNodeType.Element)
        {
            if (!string.IsNullOrWhiteSpace(content))
            {
                if(breakpending)
                {
                    Debug.Assert(string.IsNullOrEmpty(line.Value));
                    var br = CreateElement("LineBreak");
                    paragraph.Add(br);
                    breakpending = false;
                }
                line.Value += content;
        ...
        var x = (XElement) e;
        //需要产生一个新段，条件是前面的段不是空段
        if (IsBlockLevel(x) )
        {
            if(!string.IsNullOrEmpty(line.Value))
            {
                paragraph.Add(line);
                line = CreateElement("Run");
            }
        ...
        if ("a" == x.Name.LocalName && content.Length > 0)
        {
            var href = x.GetAttributeValue("href", "#");
            if(breakpending)
            {
                Debug.Assert(string.IsNullOrEmpty(line.Value));
                paragraph.Add(CreateElement("LineBreak"));
                breakpending = false;
                linelength = 0;
            }
            if(!string.IsNullOrEmpty(line.Value))
            ...
            paragraph.Add(CreateHyperlink(href, content));
            linelength += content.Length;

        }
        else if ("br" == x.Name.LocalName)
        {
            //前面已经处理了，br什么都不用做了
        }
        else if ("img" == x.Name.LocalName)
        {
            if (breakpending)
            {
                Debug.Assert(string.IsNullOrEmpty(line.Value));
            ...
            paragraph.Add(CreateFigure(x.GetAttributeValue("src", string.Empty)));
        }
        else if (x.Name.LocalName == "pre")
        {
//只能暂时忽略掉，没有很好的格式化方法
        }
        else if (x.HasElements)
        {

            for (var i = x.FirstNode; i != null; i = i.NextNode )
            {
                WriteElement(i, ref paragraph,ref line, root, ref linelength, ref breakpending, ref parano);
            }
            
        }
        else if (x.Name.LocalName != "a") //前面已经处理过a了
        {
            if (!string.IsNullOrWhiteSpace(content))
            {
                if (breakpending)
    ...
    internal float GetLinksDensity(XElement element)
    {
      ...
      //使用中文的标点符号分句
    internal int GetSegmentsCount(string s)
    {
      return s.Count(c => c == ',' || c=='，' || c == ';' || c == '；') + 1;
    }
    ...
    internal string GetInnerText(XNode node, bool dontNormalizeSpaces, bool stripinvalidxmlchars)
    {
      if (node == null)
      {
      ...
      {
          result = StripInvalidXmlChars(result);
      }
      return result;
    }
    static string StripInvalidXmlChars(string txt)
    {
    ...
    internal string GetInnerText(XNode node, bool stripinvalidxmlchars = false)
    {
      return GetInnerText(node, _dontNormalizeSpacesInTextContent, stripinvalidxmlchars);
    }
...
//        .GetElementsByTagName(articleContentElementHint)
  //      .FirstOrDefault();
    ...
    private string GetArticleContentElementHintByDivId(string url)
    {
        if (string.IsNullOrEmpty(url))
        {
        ...
        foreach (Regex urlRegex in _articleContentDivIdHints.Keys)
        {
            if (urlRegex.IsMatch(url))
            {
                return _articleContentDivIdHints[urlRegex];
            }
        }
        return _articleContentDivIdHints2 == null ?
            null : (from urlRegex in _articleContentDivIdHints2.Keys where urlRegex.IsMatch(url) select _articleContentDivIdHints2[urlRegex]).FirstOrDefault();
    }
    ...
    static int NotAsciiCharNorBlanksCount(string s)
    {
        if (string.IsNullOrEmpty(s))
            return 1;
        return s.Count(c => char.IsWhiteSpace(c) || c > 0xff) + 1;
    }
    ...
    static string _readingFlowStyle = @"<FlowDocument xmlns:x=""http://schemas.microsoft.com/winfx/2006/xaml"" ColumnWidth=""400"" xmlns=""http://schemas.microsoft.com/winfx/2006/xaml/presentation"">
