  ...
  public class NReadabilityTranscoderTests_Old
  {
    ...
    public void TestSampleInputs([Values(1, 2, 3, 4, 5, 6, 8, 9, 10, 11)]int sampleInputNumber)
    {
      ...
      string content = File.ReadAllText(string.Format(@"SampleInput\SampleInput_{0}.html", sampleInputNumberStr));
        var r = _nReadabilityTranscoder.Transcode(new TranscodingInput(content));
      bool mainContentExtracted = r.ContentExtracted;
      string transcodedContent = r.ExtractedContent;
    ...
    public void TestReplacingLinksUrls()
    {
      ...
      var r = _nReadabilityTranscoder.Transcode(new TranscodingInput(htmlContent) { Url = "http://wikipedia.org/wiki/baseArticle" });
      bool mainContentExtracted = r.ContentExtracted;
      string transcodedContent = r.ExtractedContent;
    ...
    public void TestReplacingQueryStringLinkUrls()
    {
        ...
        r =
            _nReadabilityTranscoder.Transcode(new TranscodingInput(htmlContent)
                {Url = "http://wikipedia.org/wiki/baseArticle?goodbye"});
        transcodedContent = r.ExtractedContent;
        mainContentExtracted = r.ContentExtracted;
        Assert.IsTrue(mainContentExtracted);
        Assert.IsTrue(transcodedContent.Contains("href=\"http://wikipedia.org/wiki/baseArticle?hello\""));
    }
    ...
    public void TestImageSourceTransformer()
    {
      ...
      var r = _nReadabilityTranscoder.Transcode(new TranscodingInput(htmlContent) { Url = "http://immortal.pl/" });
      bool mainContentExtracted = r.ContentExtracted;
      string transcodedContent = r.ExtractedContent;
      Assert.IsTrue(mainContentExtracted);
    ...
    {
      var r = _nReadabilityTranscoder.Transcode(new TranscodingInput("test") );
      bool mainContentExtracted = r.ContentExtracted;
      string transcodedContent = r.ExtractedContent;
    ...
    private void TestReplacingImageUrl(string srcAttribute, string url, string expectedImageUrl)
    {
      ...
      var r = _nReadabilityTranscoder.Transcode(new TranscodingInput(htmlContent){Url = url});
      bool mainContentExtracted = r.ContentExtracted;
        string transcodedContent = r.ExtractedContent;
