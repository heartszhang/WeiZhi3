...
namespace NReadability
{
    ...
    public class NReadabilityTranscoder
    {
        ...
        private static readonly Regex _VideoRegex =
            new Regex("http:\\/\\/(www\\.)?(youtube|vimeo|youku|tudou|iqiyi|video\\.sina|tv\\.sohu)\\.com",
                      RegexOptions.Compiled | RegexOptions.IgnoreCase);

        private static readonly Regex _ReplaceDoubleBrsRegex = new Regex("(<br[^>]*>[ \\n\\r\\t]*){2,}",
                                                                         RegexOptions.Compiled | RegexOptions.IgnoreCase);

        private static readonly Regex _ReplaceFontsRegex = new Regex("<(\\/?)font[^>]*>",
                                                                     RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        private static readonly Regex _ArticleTitleDashRegex3 = new Regex("[^\\|\\-_]*[\\|\\-_](.*)",
                                                                          RegexOptions.Compiled);
        ...
        private static readonly Regex _PageRegex = new Regex("pag(e|ing|inat)|([^a-z]|^)pag([^a-z]|$)",
                                                             RegexOptions.Compiled | RegexOptions.IgnoreCase);

        private static readonly Regex _LikelyParagraphDivRegex = new Regex("text|para|parbase",
                                                                           RegexOptions.Compiled |
                                                                           RegexOptions.IgnoreCase);
        ...
        public NReadabilityTranscoder(Dictionary<Regex, string> divIdHints = null)
            : this(false, false, false, divIdHints)
        {
        }
       ...
       */
            foreach (XElement linkElement in allLinks)
            {
                ...
                if (Uri.TryCreate(linkHref, UriKind.Absolute, out linkHrefUri) &&
                    linkHrefUri.Host != new Uri(articleBaseUrl).Host)
                {
                    continue;
                }
         ...
         */
                ...
                while (parentNode != null)
                {
                    ...
                    if (!negativeNodeMatch &&
                        (_NegativeWeightRegex.IsMatch(parentNodeClassAndId) ||
      ...
      */
            ...
            {
                if (possiblePagesByLink[page].Score >= 50 &&
                    (topPage == null || topPage.Score < possiblePagesByLink[page].Score))
                {
      ...
      documentBody.SetInnerHtml(bodyInnerHtml);
        ...
        {
            return new FlowDocumentTranscoder().Transcode(articleTitleElement, articleContentElement);
        }
                        ...
                        else
                        {
                            ...
                            new ChildNodesTraverser(
                                childNode =>
                                {
                                    ...
                                    XElement paraElement = new XElement("span");

                                    paraElement.Value = GetInnerText(childNode);
       ...
       * Things like preambles, content split by ads that we removed, etc. */
        ...
        internal void PrepareArticleContentElement(XElement articleContentElement)
        {
            ...
            //KillBreaks(articleContentElement);已经预先清理了
       ...
       * so remove it since we already have a header. */
            ...
            /* Remove br's that are directly before paragraphs. 已经预先清理，这里不用清理了*/
         ...
         * is more than paragraphs or other ominous signs, remove the element. */
                ...
                if (GetSegmentsCount(elementInnerText) < _MinCommaSegments)
                {
                    ...
                    bool remove = (imgsCount > psCount)
                                  ||
                                  (lisCount - _LisCountTreshold > psCount && elementNameLower != "ul" &&
                                   elementNameLower != "ol")
                                  ...
                                  ||
                                  (weight < _ClassWeightTreshold &&
                                   linksDensity > _MaxDensityForElementsWithSmallerClassWeight)
                                  ||
                                  (weight >= _ClassWeightTreshold &&
                                  ...
                                  (embedsCount > _MaxEmbedsCount ||
                                   (embedsCount == _MaxEmbedsCount &&
                                    innerTextLength < _MinInnerTextLengthInElementsWithEmbed));
...
//        .GetElementsByTagName(articleContentElementHint)
        ...
        private string GetArticleContentElementHintByDivId(string url)
        {
            ...
            return _articleContentDivIdHints2 == null
                       ? null
                       : (from urlRegex in _articleContentDivIdHints2.Keys
                          where urlRegex.IsMatch(url)
                          select _articleContentDivIdHints2[urlRegex]).FirstOrDefault();
        }
        ...
        private static int NotAsciiCharNorBlanksCount(string s)
        {
