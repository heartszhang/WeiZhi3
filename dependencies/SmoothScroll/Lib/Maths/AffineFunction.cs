namespace Logitech.Maths
{
  public class AffineFunction
  {
    ...
    #region Methods
    public double Y(double x) { return Slope * x + YIntercept; }
    public double X(double y) { return DoubleEx.IsZero(Slope) ? double.NaN : (y - YIntercept) / Slope; }

    public void TranslateTo(double ax, double ay) { YIntercept = ay - Slope * ax; }
    #endregion
  }
}
