...
namespace Bot
{
    internal static class CollectionExtensions
    {
        ...
        //{
        //    return Concat(source, items.AsEnumerable());
        //}
        ...
        public static IEnumerable<TSource> Except<TSource, TOther>(this IEnumerable<TSource> source,
                                                                   ...
                                                                   Func<TSource, TOther, bool> comparer)
        {
        ...
        /// <param name="source">The <see cref="ObservableCollection{T}"/> to create the <see cref="ObservableCollection{T}"/> from.</param>
        ...
        public static void Synchronize<TSource, TTarget>(this ObservableCollectionPlus<TTarget> targetCollection,
                                                         IList<TSource> sourceCollection,
                                                         Func<TSource, TTarget, bool> matcher,
            ...
            where TSource : IEquatable<TSource>
        {
            ...
            using (targetCollection.BeginMultiUpdate())
            {
                ...
                {
                    TSource sourceItem = sourceCollection[i];
                    int targetIndex = targetCollection.IndexOf(targetItem => matcher(sourceItem, targetItem));
                    if (targetIndex >= 0)
                    {
                        if (targetIndex != i)
                        {
                            Debug.Assert(targetIndex > i,
                                         "this would only happen if we have duplicates...which we should never have!");
                    ...
                    {
                        TTarget newItem = mapper(sourceItem);
